...
declare result json;
begin
  -- auth.role() = 'anon'          → anon/publishable key, no JWT user — block.
  -- auth.role() = 'authenticated' → JWT user present — allow only if is_admin().
  -- auth.role() IS NULL           → direct postgres/service_role call — allow.
