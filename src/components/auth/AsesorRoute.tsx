...
/**
 * Gate para el panel de asesores (/asesor/*).
 * La barrera REAL es RLS en la DB; este guard es solo UX:
 *  - sin sesión        -> redirige a /login
 *  - sesión no-asesor  -> pantalla de acceso restringido
 *  - sesión asesor     -> renderiza el panel
 * "Asesor" se valida con la RPC SECDEF is_asesor (no con un query directo
 * a usuarios_sistema, que RLS bloquea).
 *
 * IMPORTANT: preserves the deadlock fix from AdminRoute:
 * never await supabase RPC inside onAuthStateChange callback.
 * Use setTimeout(0) + active flag to defer evaluation.
 */
export function AsesorRoute({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const evaluate = async (session: Session | null) => {
      ...
      const { data, error } = await (supabase as any).rpc("is_asesor");
      if (!active) return;
      ...
      <div className="min-h-screen bg-[#0B1A1E] text-white flex items-center justify-center p-6">
        <div className="max-w-sm text-center space-y-4">
          ...
          <p className="text-[#94B3BB] text-sm">
            Tu cuenta no tiene permisos de asesor para este panel.
          </p>
          <button
