...
interface StatsResult { sessions: number; pageviews: number; avg_active_ms: number; avg_max_scroll: number; }
interface ScrollPoint { pct: number; sessions_reached: number; pct_reached: number; }
interface SectionRow { section: string; avg_active_ms: number; appearances: number; }
interface ClickPoint { x: number; y: number; value: number; }
interface KnownPath { path: string; pageviews: number; }
interface Filter { path: string; from: string; to: string; }
...
// Leads per period (conversion ratio vs. sessions)
async function fetchLeadsCount(f: Filter): Promise<number> {
  const { count, error } = await supabase
    .from("leads")
    .select("id", { count: "exact", head: true })
    .gte("created_at", toIso(new Date(f.from)))
    .lte("created_at", toIso(new Date(f.to + "T23:59:59")));
...
function ClickHeatmap({ points }: { points: ClickPoint[] }) {
  ...
  return (
    <div className="relative w-full" style={{ height: 460 }}>
      ...
      <div ref={containerRef} className="absolute inset-0 rounded-lg z-10" style={{ background: "transparent" }} />
      {points.length === 0 && (
...
export default function AnalyticsContent() {
  ...
  const { data: paths = [] } = useQuery({
    ...
    queryFn: () => fetchPaths(filter), staleTime: 60_000,
  });
  const { data: stats, isFetching: statsLoading } = useQuery({
    queryKey: ["analytics-stats", filter], queryFn: () => fetchStats(filter), staleTime: 60_000,
  });
  const { data: scrollDist = [] } = useQuery({
    queryKey: ["analytics-scroll", filter], queryFn: () => fetchScrollDist(filter), staleTime: 60_000,
  });
  const { data: sections = [] } = useQuery({
    queryKey: ["analytics-sections", filter], queryFn: () => fetchSections(filter), staleTime: 60_000,
  });
  const { data: clicks = [] } = useQuery({
    queryKey: ["analytics-clicks", filter], queryFn: () => fetchClicks(filter), staleTime: 60_000,
  });
  const { data: leadsCount = 0 } = useQuery({
    queryKey: ["analytics-leads-count", filter], queryFn: () => fetchLeadsCount(filter), staleTime: 60_000,
  });

  const convRate = stats && stats.sessions > 0
    ? ((leadsCount / stats.sessions) * 100).toFixed(2)
    : "0";

  // Exit point: scroll % at which 50% of sessions have abandoned
  let exitPoint = "—";
  if (scrollDist.length > 0) {
    const half = scrollDist.find((p) => p.pct_reached <= 50);
    if (half) exitPoint = `${half.pct}%`;
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-[#0d1f26] border border-[#1d9fa9]/20 rounded-xl p-5 flex flex-wrap gap-4 items-end">
        ...
        {statsLoading && <span className="text-xs text-[#4A8A94] self-center animate-pulse">Cargando…</span>}
      </div>

      {/* Stat cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Sesiones" value={stats ? fmtNum(stats.sessions) : "—"} sub={filter.path || "Todas las páginas"} />
        <StatCard label="Tiempo activo promedio" value={stats ? fmtMs(stats.avg_active_ms) : "—"} sub="Solo pestaña activa" />
        <StatCard label="Leads recibidos" value={fmtNum(leadsCount)} sub={`Conversión: ${convRate}%`} />
        <StatCard label="Punto medio de salida" value={exitPoint} sub="50% de sesiones abandonan aquí" />
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Páginas vistas" value={stats ? fmtNum(stats.pageviews) : "—"} />
        <StatCard label="Scroll promedio" value={stats ? `${Math.round(stats.avg_max_scroll)}%` : "—"} sub="Profundidad máxima" />
        <StatCard
          label="Páginas/sesión"
          value={stats && stats.sessions > 0 ? (stats.pageviews / stats.sessions).toFixed(2) : "—"}
        />
        <StatCard
          label="Sesiones rebote"
          value={stats && scrollDist.length > 0
            ? `${Math.round(100 - (scrollDist.find((p) => p.pct === 30)?.pct_reached ?? 0))}%`
            : "—"}
          sub="No pasaron del 30% del scroll"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[#0d1f26] border border-[#1d9fa9]/20 rounded-xl p-5">
          <h2 className="text-sm font-semibold text-white mb-1">Curva de abandono · dónde se va la gente</h2>
          <p className="text-xs text-[#4A8A94] mb-5">
            % de sesiones que llegaron a cada profundidad de scroll. La caída más pronunciada = punto de fuga.
          </p>
          {scrollDist.length > 0 ? (
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={scrollDist} margin={{ top: 4, right: 16, bottom: 0, left: 0 }}>
                ...
                <XAxis dataKey="pct" tickFormatter={(v) => `${v}%`} tick={{ fill: "#4A8A94", fontSize: 11 }} />
                <YAxis tickFormatter={(v) => `${v}%`} tick={{ fill: "#4A8A94", fontSize: 11 }} domain={[0, 100]} />
                <Tooltip
                ...
                />
                <Area type="monotone" dataKey="pct_reached" stroke="#1d9fa9" strokeWidth={2} fill="url(#grad)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          ) : <Empty />}
        </div>
        ...
        <div className="bg-[#0d1f26] border border-[#1d9fa9]/20 rounded-xl p-5">
          <h2 className="text-sm font-semibold text-white mb-1">Atención por sección · dónde dura más</h2>
          <p className="text-xs text-[#4A8A94] mb-5">
            Tiempo activo promedio en cada sección de la página
          </p>
          {sections.length > 0 ? (
            <ResponsiveContainer width="100%" height={260}>
              <BarChart
              ...
              >
                ...
                <XAxis type="number" tickFormatter={(v) => fmtMs(v)} tick={{ fill: "#4A8A94", fontSize: 11 }} />
                <YAxis type="category" dataKey="section" tick={{ fill: "#b0cdd3", fontSize: 11 }} width={110} />
                <Tooltip
        ...
        <div className="flex items-start justify-between mb-1">
          <div>
            ...
            <p className="text-xs text-[#4A8A94] mt-1">
              X = % ancho del viewport · Y = % del largo total de la página
            </p>
          </div>
